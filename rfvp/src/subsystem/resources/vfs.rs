...
impl VfsFile {
    ...
    pub fn read_file(&self, name: &str) -> Result<Vec<u8>> {
        ...
        let path = app_base_path().join(&self.folder_name).join(name);
        if path.get_path().exists() {
            let mut file = File::open(path.get_path())?;
            let mut buffer = vec![];
        ...
        let entry = self.entries.get(name).ok_or_else(|| {
            anyhow::anyhow!("File not found in VFS: {}", name)
        })?;
