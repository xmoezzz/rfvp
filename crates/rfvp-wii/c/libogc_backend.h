#ifndef RFVP_LIBOGC_BACKEND_H
#define RFVP_LIBOGC_BACKEND_H
...
enum {
    RFVP_WII_OK = 0,
    RFVP_WII_IO = -1,
    RFVP_WII_NOT_FOUND = -2,
    RFVP_WII_INVALID_DATA = -3,
    RFVP_WII_INVALID_ARGUMENT = -4,
    RFVP_WII_UNSUPPORTED = -5,
    RFVP_WII_OUT_OF_MEMORY = -6,
    RFVP_WII_CAPACITY_EXCEEDED = -7,
    RFVP_WII_END_OF_FILE = -8,
    RFVP_WII_BACKEND = -9
};
...
typedef void (*RawWiiLogFn)(void *, uint32_t, const uint8_t *, size_t);
typedef void (*RawWiiFatalFn)(void *, uint32_t, const uint8_t *, size_t);

typedef struct RawWiiHost {
    void *fs_ctx;
...
int rfvp_wii_app_main(const RawWiiHost *host);
int rfvp_wii_app_push_key(void *app, uint32_t key_id, int pressed);
int rfvp_wii_app_push_quit(void *app);
int rfvp_wii_app_push_pointer_move(void *app, int32_t x, int32_t y);
int rfvp_wii_app_push_pointer_button(void *app, uint32_t button_id, int pressed, int32_t x, int32_t y);
