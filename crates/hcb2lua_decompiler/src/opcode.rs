...
impl TryFrom<u8> for Opcode {
    ...
    fn try_from(v: u8) -> core::result::Result<Self, Self::Error> {
        match v {
            ...
            x if x == Opcode::InitStack as u8 => Ok(Opcode::InitStack),
            x if x == Opcode::Call as u8 => Ok(Opcode::Call),
            x if x == Opcode::Syscall as u8 => Ok(Opcode::Syscall),
            x if x == Opcode::Ret as u8 => Ok(Opcode::Ret),
            x if x == Opcode::RetV as u8 => Ok(Opcode::RetV),
            x if x == Opcode::Jmp as u8 => Ok(Opcode::Jmp),
            x if x == Opcode::Jz as u8 => Ok(Opcode::Jz),
            x if x == Opcode::PushNil as u8 => Ok(Opcode::PushNil),
            x if x == Opcode::PushTrue as u8 => Ok(Opcode::PushTrue),
            x if x == Opcode::PushI32 as u8 => Ok(Opcode::PushI32),
            x if x == Opcode::PushI16 as u8 => Ok(Opcode::PushI16),
            x if x == Opcode::PushI8 as u8 => Ok(Opcode::PushI8),
            x if x == Opcode::PushF32 as u8 => Ok(Opcode::PushF32),
            x if x == Opcode::PushString as u8 => Ok(Opcode::PushString),
            x if x == Opcode::PushGlobal as u8 => Ok(Opcode::PushGlobal),
            x if x == Opcode::PushStack as u8 => Ok(Opcode::PushStack),
            x if x == Opcode::PushGlobalTable as u8 => Ok(Opcode::PushGlobalTable),
            x if x == Opcode::PushLocalTable as u8 => Ok(Opcode::PushLocalTable),
            x if x == Opcode::PushTop as u8 => Ok(Opcode::PushTop),
            x if x == Opcode::PushReturn as u8 => Ok(Opcode::PushReturn),
            x if x == Opcode::PopGlobal as u8 => Ok(Opcode::PopGlobal),
            x if x == Opcode::PopStack as u8 => Ok(Opcode::PopStack),
            x if x == Opcode::PopGlobalTable as u8 => Ok(Opcode::PopGlobalTable),
            x if x == Opcode::PopLocalTable as u8 => Ok(Opcode::PopLocalTable),
            x if x == Opcode::Neg as u8 => Ok(Opcode::Neg),
            ...
            x if x == Opcode::BitTest as u8 => Ok(Opcode::BitTest),
            x if x == Opcode::And as u8 => Ok(Opcode::And),
            x if x == Opcode::Or as u8 => Ok(Opcode::Or),
            x if x == Opcode::SetE as u8 => Ok(Opcode::SetE),
            x if x == Opcode::SetNE as u8 => Ok(Opcode::SetNE),
            x if x == Opcode::SetG as u8 => Ok(Opcode::SetG),
            x if x == Opcode::SetLE as u8 => Ok(Opcode::SetLE),
            x if x == Opcode::SetL as u8 => Ok(Opcode::SetL),
            x if x == Opcode::SetGE as u8 => Ok(Opcode::SetGE),
            _ => Err(()),
        }
    }
}
