...
unsafe extern "C" {
    fn rfvp_ps3_platform_present_rgba8(pixels: *const RawRgba8, width: u32, height: u32) -> i32;
}
...
impl Default for PS3Renderer {
    fn default() -> Self {
        Self::new()
    }
}

impl RfvpRenderer for PS3Renderer {
    fn create_texture(
        &mut self,
        id: TextureId,
        desc: TextureDesc,
    ...
    fn present(&mut self) -> RfvpResult<()> {
        ...
        let status = unsafe {
            rfvp_ps3_platform_present_rgba8(pixels.as_ptr(), DISPLAY_WIDTH, DISPLAY_HEIGHT)
        };
        if status == 0 {
            Ok(())
        } else {
            Err(crate::status::ps3_status_to_rfvp_error(status))
        }
    }
}
