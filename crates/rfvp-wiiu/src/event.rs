...
unsafe extern "C" {
    fn rfvp_wiiu_platform_poll_input(out_state: *mut RawWiiUInputState) -> i32;
}

pub struct WiiUEventQueue {
...
impl WiiUInput {
    ...
    pub fn poll(&mut self, queue: &mut WiiUEventQueue) -> RfvpResult<()> {
        let mut state = RawWiiUInputState::default();
        let status = unsafe { rfvp_wiiu_platform_poll_input(&mut state) };
        if status != 0 {
            return Err(crate::status::wiiu_status_to_rfvp_error(status));
        }
