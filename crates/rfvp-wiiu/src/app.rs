...
use crate::event::{WiiUEventQueue, WiiUInput};
use crate::host::WiiUHost;

pub struct WiiUApp {
    core: RfvpCore,
    host: WiiUHost,
    events: WiiUEventQueue,
    input: WiiUInput,
}
...
impl WiiUApp {
    pub fn new(config: RfvpCoreConfig, event_capacity: usize) -> RfvpResult<Self> {
        Ok(Self {
            ...
            host: WiiUHost::new(),
            events: WiiUEventQueue::new(event_capacity),
            input: WiiUInput::new()?,
        })
    }
    ...
    pub fn host(&mut self) -> &mut WiiUHost {
        &mut self.host
    }
