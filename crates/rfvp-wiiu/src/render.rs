...
unsafe extern "C" {
    fn rfvp_wiiu_platform_present_rgba8(pixels: *const RawRgba8, width: u32, height: u32) -> i32;
}
...
pub struct WiiURenderer {
    frame_width: u32,
    frame_height: u32,
    frame_index: u64,
    framebuffer: Vec<ColorU8>,
    display_pixels: Vec<RawRgba8>,
...
impl Default for WiiURenderer {
    fn default() -> Self {
        Self::new()
    }
}

impl RfvpRenderer for WiiURenderer {
    fn create_texture(
        &mut self,
        id: TextureId,
        desc: TextureDesc,
    ...
    fn present(&mut self) -> RfvpResult<()> {
        let pixels = self.prepare_display_pixels()?;
        let status = unsafe {
            rfvp_wiiu_platform_present_rgba8(pixels.as_ptr(), DISPLAY_WIDTH, DISPLAY_HEIGHT)
        };
        if status == 0 {
            Ok(())
        } else {
            Err(crate::status::wiiu_status_to_rfvp_error(status))
        }
    }
}
