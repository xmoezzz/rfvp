...
impl SpritePipeline {
    pub fn new(device: &wgpu::Device, queue: &Arc<wgpu::Queue>, layouts: &BindGroupLayouts, target_format: wgpu::TextureFormat) -> Self {
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("rfvp_render.sprite_shader.wasm"),
            source: wgpu::ShaderSource::Wgsl(include_str!("sprite_wasm.wgsl").into()),
        });

        let pc_bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("rfvp_render.sprite_pc_bind_group_layout.wasm"),
        ...
        let pc_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("rfvp_render.sprite_pc_uniform.wasm"),
            size: PC_RING_SIZE,
        ...
        let pc_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("rfvp_render.sprite_pc_bind_group.wasm"),
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("rfvp_render.sprite_pipeline_layout.wasm"),
            bind_group_layouts: &[&layouts.texture, &pc_bind_group_layout],
        ...
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("rfvp_render.sprite_pipeline.wasm"),
    ...
    pub fn draw<'a>(
        ...
        self.queue
            .write_buffer(&self.pc_buffer, pc_offset as u64, bytemuck::bytes_of(&m));
        pass.set_bind_group(1, &self.pc_bind_group, &[pc_offset]);
