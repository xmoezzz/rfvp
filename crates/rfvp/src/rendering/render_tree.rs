...
impl PrimitiveAnimations {
    pub fn apply(&self, base: &mut Transform2D, base_alpha: &mut f32, now: f64) {
        if let Some(tr) = &self.pos {
            base.position = tr.value_at(now).0;
        }
        if let Some(sc) = &self.scale {
            base.scale = sc.value_at(now).0;
        }
        if let Some(rt) = &self.rot {
            base.rotation = rt.value_at(now).0;
        }
        if let Some(op) = &self.opacity {
            *base_alpha = op.value_at(now).0;
        }
    }
}
...
impl Default for PrimitiveKind {
    fn default() -> Self {
        PrimitiveKind::ColorRect(ColorRectPrimitive {
...
impl RenderTree {
    ...
    pub fn assign_ids_recursive(&mut self) {
        Self::assign_ids(&mut self.root, &mut self.next_id);
    }
    fn assign_ids(node: &mut Node, next: &mut u64) {
        node.id = NodeId(*next);
        *next += 1;
        for ch in &mut node.children {
            Self::assign_ids(ch, next);
        }
    }
}
...
impl Resources {
    ...
    pub fn image_tex_id(&self, index: u32) -> Option<TextureId> {
        self.image_tex_id.get(&index).copied()
    }
    ...
    pub fn update_videos(&mut self, dt: f32) {
        for (_idx, (src, tex)) in self.videos.iter_mut() {
            if let Some(frame) = src.update(dt) {
                let color_image = ept::image::ColorImage {
...
impl ParticleEmitter {
    pub fn new(spec: ParticleEmitterSpec) -> Self {
        Self {
            spec,
            particles: Vec::new(),
            acc: 0.0,
...
fn flatten_node(
    ...
    for prim in &node.primitives {
        ...
        match &prim.kind {
            ...
            PrimitiveKind::Text(tx) => {
                ...
                let aabb = Aabb {
                    min: world_pos,
                    max: world_pos + tx_estimate_aabb(tx, t),
                };
                if aabb.intersects(ctx.viewport) {
            ...
            PrimitiveKind::Particles(pp) => {
                ...
                for p in em.particles.iter() {
                    let corners = transformed_quad(
                        p.size,
                        &(world2 * Affine2::translate(p.pos)),
                        Vec2::splat(0.5) * p.size,
                    );
                    let aabb = corners_aabb(&corners);
                    if aabb.intersects(ctx.viewport) {
    ...
    for ch in &node.children {
        flatten_node(ch, world, base_z, ctx, res, now, dt);
    }
}
...
fn tx_estimate_aabb(tx: &TextPrimitive, _t: Transform2D) -> Vec2 {
    ...
    let h = tx.font.size;
    let w = if let Some(wrap) = tx.wrap_width {
        wrap
    } else {
        tx.text.len() as f32 * (tx.font.size * 0.6)
    };
    Vec2::new(w, h)
}
...
impl Renderer {
    ...
    pub fn update_and_draw(&mut self, painter: &Painter, tree: &RenderTree, now: f64, dt: f32) {
        ...
        for d in fctx.drawables {
            match d.kind {
                DrawableKind::Mesh { mesh, .. } => {
                    painter.add(ept::Shape::mesh(mesh));
                }
                DrawableKind::Text {
                    pos,
                    text,
