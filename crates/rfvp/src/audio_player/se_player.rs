...
pub struct SePlayer {
    ...
    se_tracks: [AudioTrackHandle; SE_SLOT_COUNT],
...
impl SePlayer {
    pub fn new(audio_manager: Arc<AudioManager>) -> Self {
        let se_tracks = [(); SE_SLOT_COUNT].map(|_| audio_manager.create_track());
    ...
    pub fn load(&mut self, slot: i32, se: Vec<u8>) -> anyhow::Result<()> {
        ...
        let sound = self.audio_manager.load_sound(se)?;
    ...
    pub fn set_panning(&mut self, slot: i32, pan: f64, tween: Tween) {
        ...
        if let Some(handle) = self.se_slots[slot].as_mut() {
            self.audio_manager.set_handle_panning(handle, pan as f32, tween);
        } else {
            warn!(
