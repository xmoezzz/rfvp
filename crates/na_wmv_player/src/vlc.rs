...
impl VlcTable {
    ...
    pub fn build(codes: &[(u32, u8, i32)], max_bits: u8) -> Self {
        ...
        for &(_bits, _len, sym) in codes {
            ...
            if last {
                if run < 64 {
                    max_level_last[run] = max_level_last[run].max(level as u8);
                }
                if level < 64 {
                    max_run_last[level] = max_run_last[level].max(run as u8);
                }
            } else {
                if run < 64 {
                    max_level_nolast[run] = max_level_nolast[run].max(level as u8);
                }
                if level < 64 {
                    max_run_nolast[level] = max_run_nolast[level].max(run as u8);
                }
            }
        }
        VlcTable {
    ...
    #[inline]
    pub fn max_level(&self, run: usize, last: bool) -> u8 {
        let r = run.min(63);
        if last {
            self.max_level_last[r]
        } else {
            self.max_level_nolast[r]
        }
    }
    /// Max run coded for given (level, last) in this table.
    #[inline]
    pub fn max_run(&self, level: usize, last: bool) -> u8 {
        let l = level.min(63);
        if last {
            self.max_run_last[l]
        } else {
            self.max_run_nolast[l]
        }
    }
}
...
pub fn unpack_rl(v: i32) -> (u8, u8, bool) {
    (((v >> 8) & 0x3F) as u8, ((v >> 1) & 0x7F) as u8, v & 1 != 0)
}
...
pub fn subblkpat_vlc() -> VlcTable {
    const T: &[(u32, u8, i32)] = &[(0b1, 1, 3), (0b01, 2, 2), (0b001, 3, 1), (0b0001, 4, 0)];
    VlcTable::build(T, 4)
}
...
pub fn mv_diff_vlc_k0() -> VlcTable {
    mv_diff_vlc(0)
}
pub fn mv_diff_vlc_k1() -> VlcTable {
    mv_diff_vlc(1)
}
...
pub fn wmv2_cbpy_vlc() -> VlcTable {
    VlcTable::build(WMV2_CBPY_CODES, 6)
}
pub fn wmv2_cbpc_p_vlc() -> VlcTable {
    VlcTable::build(WMV2_CBPC_P_CODES, 6)
}
