...
use crate::subsystem::components::{
...
fn add_font_to_atlas_if_missing(size: usize, color: &Color, font_path: &str,  font_atlas: &mut AtomicRefMut<FontAtlas>) {
    if let None = font_atlas.get_texture(&font_path, size, &color) {
        ...
        let res = crate::subsystem::resources::font_atlas::generate_bitmap(Font::TrueType { font_path: font_path.to_string() },
                                                                      size,
                                                                      &color);
...
fn update_bitmap(texture_path: String,
    ...
    mod tests {
        ...
        use crate::subsystem::resources::asset_manager::{AssetManager, AssetRef};
