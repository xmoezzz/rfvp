use crate::subsystem::components::color::Color;
use crate::subsystem::components::material::Texture;
...
const TEXT: &str =
    "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890éèàùç-?!.,:=/+-%&'()";
...
pub(crate) fn generate_bitmap(
    font: crate::subsystem::components::ui::font::Font,
    font_size: usize,
    font_color: &Color,
) -> anyhow::Result<TrueTypeData> {
    if let crate::subsystem::components::ui::font::Font::TrueType { font_path } = font {
        return match read_file(Path::new(&font_path)) {
            Ok(res) => {
                ...
                if let Ok(font_vec) = font {
                    ...
                    for (pos, glyph) in glyphs.drain(0..glyphs.len()).enumerate() {
                        if let Some(outlined) = scaled_font.outline_glyph(glyph) {
                            ...
                            let char_pos = CharacterPosition::new(
                                bounds.min.x,
                                bounds.min.y,
                                bounds.max.x,
                                bounds.max.y,
                            ...
                            character_positions
                                .insert(TEXT.to_string().chars().nth(pos).unwrap(), char_pos);
                        }
                    }
                ...
                anyhow::bail!("Impossible to read font")
            }
            Err(_) => anyhow::bail!("Impossible to find font file"),
        };
    }

    anyhow::bail!("Wrong type sent to bitmap generation")
}
